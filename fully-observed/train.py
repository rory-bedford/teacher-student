"""
Fully-observed student training with CMA-ES initialisation.

Student and teacher networks are identical (no hidden neurons, no EM).
A global scaling factor perturbation is applied to the weights, and the
student must recover the correct scaling factors.
...
def main(
    ...
    loss_weight_firing_rate = hyperparameters.loss_weight.firing_rate
    ...
    if resume_from is not None:
        ...
        n_feedforward = n_total_inputs - n_neurons
    ...
    # ============================================================
    # Cell-type name lists for projection registries
    # ============================================================
    # The chunked-FF model treats both true-FF inputs and recurrent inputs
    # (from previous chunk's spikes) uniformly as feedforward; the combined
    # input-cell-type space is FF names followed by recurrent names.
    rec_cell_type_names = list(recurrent.cell_types.names)
    ff_cell_type_names = list(feedforward.cell_types.names[:n_ff_cell_types])

    # Apply concatenated_mask to the perturbed weights up-front so projection
    # builders see zeros on the inactive connections. The mask is preserved
    # via the FrozenProjection's stored block (zeros stay zero) and through
    # ScalingFactorProjection's connectome buffer in the gradient phase.
    masked_perturbed_weights = perturbed_weights * concatenated_mask.astype(np.float32)
    ...
    if wandb_config:
        ...
        run_name = wandb_config.pop("name", None) or (
            output_dir.name if output_dir else "fully-observed"
        )
    ...
    if resume_from is not None and cma_state_path.exists():
        ...
        # Inference-only CMA-ES model: every projection is a FrozenProjection
        # with the current scaling factors baked into its weight block. CMA-ES
        # rewrites the FrozenProjection buffers (and refreshes the simulator's
        # cached connection_weights_k) each evaluation — see cma_update_fn.
        ff_block_init = masked_perturbed_weights[:n_feedforward, :]
        rec_block_init = masked_perturbed_weights[n_feedforward:, :]
        cma_projections = make_frozen_chunked_ff_projections(
            rec_weights=rec_block_init,
            ff_weights=ff_block_init,
            cell_type_indices=cell_type_indices,
        ...
        print("\nCMA-ES model initialized (FrozenProjections, baked SFs):")
        print(f"  - Output neurons: {n_neurons}")
        print(f"  - {n_total_inputs} feedforward inputs per neuron")
        print(f"  - {len(cma_projections)} projection pairs")

        sf_shape = concatenated_scaling_factors.shape
        n_ff_ct = len(ff_cell_type_names)

        # Per-pair source/target index lookups (computed once) for rebaking.
        _ff_idx_per_ct = {
            name: np.flatnonzero(
                concatenated_cell_type_indices[:n_feedforward] == ct_id
            )
            for ct_id, name in enumerate(ff_cell_type_names)
        }
        _rec_idx_per_ct = {
            name: np.flatnonzero(cell_type_indices == ct_id)
            for ct_id, name in enumerate(rec_cell_type_names)
        }
        _ff_block_base = ff_block_init.astype(np.float32)
        _rec_block_base = rec_block_init.astype(np.float32)

        # Track the most-recent SFs written into the CMA model, so the
        # callback (and post-search readout) can recover them without
        # relying on cma_model.scaling_factors_FF (which is all-ones for
        # a FrozenProjection-only model).
        ...
        def cma_update_fn(flat_log_params: np.ndarray) -> None:
            ...
            # Rebake each FrozenProjection's weight buffer in-place with the
            # new per-pair scaling factor.
            for src_id, src_name in enumerate(ff_cell_type_names):
                src_idx = _ff_idx_per_ct[src_name]
                for tgt_id, tgt_name in enumerate(rec_cell_type_names):
                    tgt_idx = _rec_idx_per_ct[tgt_name]
                    block = _ff_block_base[np.ix_(src_idx, tgt_idx)] * float(
                        sf_np[src_id, tgt_id]
                    )
                    proj = cma_model.projections[f"{src_name}__{tgt_name}"]
                    proj._weights.copy_(
                        torch.from_numpy(block).to(proj._weights.device)
                    )
            for src_id, src_name in enumerate(rec_cell_type_names):
                src_idx = _rec_idx_per_ct[src_name]
                for tgt_id, tgt_name in enumerate(rec_cell_type_names):
                    tgt_idx = _rec_idx_per_ct[tgt_name]
                    block = _rec_block_base[np.ix_(src_idx, tgt_idx)] * float(
                        sf_np[n_ff_ct + src_id, tgt_id]
                    )
                    proj = cma_model.projections[f"{src_name}__{tgt_name}"]
                    proj._weights.copy_(
                        torch.from_numpy(block).to(proj._weights.device)
                    )
            # Refresh cached connection_weights_k buffers (assembled from
            # projections at init by _precompute_weight_products).
            cma_model._precompute_weight_products()
        ...
        def cma_es_callback(metrics):
            ...
            # FrozenProjection-only model: track SFs externally (see
            # current_cma_sf, updated by cma_update_fn).
            ...
            # cma_model already has best params written by EvolutionarySearch
        ...
        # EvolutionarySearch.search() writes the best candidate into the model
        # at the end via update_fn, which also updated current_cma_sf.
        best_scaling_factors_FF = current_cma_sf.copy()
    ...
    # Build gradient-phase projections per (input_ct, output_ct) pair.
    # optimisable="scaling_factors" → one trainable scalar per pair, on top of
    # the perturbed connectome (baseline). For any other value we error: this
    # script is currently only exercised with scaling_factors.
    if optimisable != "scaling_factors":
        raise NotImplementedError(
            f"Projection migration only handles optimisable='scaling_factors', "
            f"got {optimisable!r}. Extend the per-pair builder below if needed."
        )

    n_ff_ct = len(ff_cell_type_names)
    ff_idx_per_ct = {
        name: np.flatnonzero(concatenated_cell_type_indices[:n_feedforward] == ct_id)
        for ct_id, name in enumerate(ff_cell_type_names)
    }
    rec_idx_per_ct = {
        name: np.flatnonzero(cell_type_indices == ct_id)
    ...
    ff_block_unscaled = masked_perturbed_weights[:n_feedforward, :].astype(np.float32)
    rec_block_unscaled = masked_perturbed_weights[n_feedforward:, :].astype(np.float32)

    grad_projections: dict = {}
    for src_id, src_name in enumerate(ff_cell_type_names):
        src_idx = ff_idx_per_ct[src_name]
        for tgt_id, tgt_name in enumerate(rec_cell_type_names):
            tgt_idx = rec_idx_per_ct[tgt_name]
            connectome = ff_block_unscaled[np.ix_(src_idx, tgt_idx)]
            init_sf = float(best_scaling_factors_FF[src_id, tgt_id])
            grad_projections[(src_name, tgt_name)] = ScalingFactorProjection(
    ...
    for src_id, src_name in enumerate(rec_cell_type_names):
        ...
        for tgt_id, tgt_name in enumerate(rec_cell_type_names):
            tgt_idx = rec_idx_per_ct[tgt_name]
            connectome = rec_block_unscaled[np.ix_(src_idx, tgt_idx)]
            init_sf = float(best_scaling_factors_FF[n_ff_ct + src_id, tgt_id])
            grad_projections[(src_name, tgt_name)] = ScalingFactorProjection(
                connectome=connectome,
                init_sf=init_sf,
            )
    ...
    model = FeedforwardConductanceLIFNetwork(
        **model_kwargs,
        projections=grad_projections,
    )
    model.to(device)

    print(f"\nGradient model initialized (optimisable={optimisable}):")
    print(f"  - Output neurons: {n_neurons}")
    print(f"  - {n_total_inputs} feedforward inputs per neuron")
    ...
    def plot_generator(
        ...
        **kwargs,
        ...
        fig = plot_spike_trains(
            ...
            fraction=1.0,
            random_seed=None,
            title=f"Fully-observed: Target vs Trained (first {n_plot} neurons)",
            ylabel="Neuron",
            figsize=(14, 8),
        )
    ...
    # Gradient phase uses only Van Rossum loss (CMA-ES uses all losses)
