...
Writes, next to this script:
    fig07_summary.csv   reconstructed_fraction, recorded_pool_fraction, kappa, n_free_params, n_in_loss,
                        seed, evaluation{held_out,perturbation}, group, cell_type, n_cells, metric,
                        value, ceiling_value
    fig07_rates.csv     reconstructed_fraction, recorded_pool_fraction, neuron_id, cell_type, group, seed,
                        teacher_rate_hz, student_rate_hz, fluctuation_r2
    fig07_spikes.csv    reconstructed_fraction, neuron_id, group, seed, source, time_s
...
def main(runs_dir, out_dir):
    ...
    summary.to_csv(out_dir / "fig07_summary.csv", index=False)
    pd.DataFrame(rates).to_csv(out_dir / "fig07_rates.csv", index=False)
    pd.DataFrame(spikes).to_csv(out_dir / "fig07_spikes.csv", index=False)
