...
This script is a control variant of the varying-noise experiment. Instead of
adding noise to the weight matrix, it SHUFFLES non-zero weights within each
(pre_type, post_type) cell-type block.

This preserves:
  - The connectivity mask (which synapses exist)
  - The weight distribution per cell-type pair (same values, same statistics)

This destroys:
  - Which specific synapse has which weight value (synapse identity)

The purpose is to test whether learned scaling factors exploit specific
connectome structure, or whether they can compensate for any weight matrix
with the right block-level statistics.
...
def shuffle_weights_by_cell_type_pair(
    weights, pre_cell_type_indices, post_cell_type_indices, rng
):
    """Shuffle non-zero weights within each (pre_type, post_type) block.

    Preserves: connectivity mask, weight distribution per block.
    Destroys: which specific synapse has which weight value.
    """
    shuffled = weights.copy()
    for pre_type in np.unique(pre_cell_type_indices):
        for post_type in np.unique(post_cell_type_indices):
            pre_mask = pre_cell_type_indices == pre_type
            post_mask = post_cell_type_indices == post_type
            block = shuffled[np.ix_(pre_mask, post_mask)]
            nonzero = block != 0
            vals = block[nonzero].copy()
            rng.shuffle(vals)
            block[nonzero] = vals
            shuffled[np.ix_(pre_mask, post_mask)] = block
    return shuffled
...
def main(
    ...
    # ================================================================
    # FRESH START PATH: Shuffle weights, apply perturbation, save initial state
    # ================================================================
    else:
        print("\n" + "=" * 60)
        print("FRESH START: Shuffling weights and applying perturbations")
        print("=" * 60)

        # Create RNG for weight shuffling
        ...
        # ===============================================
        # Shuffle Weights Within Cell-Type Pairs
        # ===============================================

        print("\nShuffling weights within cell-type pairs...")

        original_feedforward_weights = feedforward_weights.copy()
        original_recurrent_weights = weights.copy()

        # Shuffle feedforward weights (feedforward_cell_types x recurrent_cell_types)
        feedforward_weights = shuffle_weights_by_cell_type_pair(
            feedforward_weights,
            feedforward_cell_type_indices,
            cell_type_indices,
            shuffle_rng,
        ...
        # Shuffle recurrent weights (recurrent_cell_types x recurrent_cell_types)
        weights = shuffle_weights_by_cell_type_pair(
            weights,
            cell_type_indices,
            cell_type_indices,
            shuffle_rng,
        ...
        # Also concatenate original weights for comparison
        original_concatenated_weights = np.concatenate(
            [original_feedforward_weights, original_recurrent_weights], axis=0
        )

        # Print per-block shuffle statistics
        print("\n  Per cell-type pair statistics (should match before/after):")
        for in_type in range(n_input_types):
            for out_type in range(n_output_types):
                ...
                orig_pair = original_concatenated_weights[pair_mask]
                shuf_pair = concatenated_weights[pair_mask]
                nonzero = orig_pair != 0

                if nonzero.sum() == 0:
                    continue

                orig_nz = orig_pair[nonzero]
                shuf_nz = shuf_pair[nonzero]
                corr = np.corrcoef(orig_nz, shuf_nz)[0, 1]
                print(
                    f"    [{in_type}->{out_type}] n={nonzero.sum():,}: "
                    f"orig mu={orig_nz.mean():.4f} std={orig_nz.std():.4f}, "
                    f"shuf mu={shuf_nz.mean():.4f} std={shuf_nz.std():.4f}, "
                    f"corr={corr:.4f}"
                )

        # Global stats
        nonzero_mask = concatenated_weights != 0
        original_nz = original_concatenated_weights[nonzero_mask]
        shuffled_nz = concatenated_weights[nonzero_mask]
        global_corr = np.corrcoef(original_nz, shuffled_nz)[0, 1]
        print(
            f"\n  - Global original (non-zero): mean={original_nz.mean():.6f}, std={original_nz.std():.6f}"
        )
        print(
            f"  - Global shuffled (non-zero): mean={shuffled_nz.mean():.6f}, std={shuffled_nz.std():.6f}"
        )
        print(f"  - Global correlation: r={global_corr:.4f}")

        # Create and save weight shuffle comparison plot
        fig, axes = plt.subplots(1, 2, figsize=(14, 6))

        ax = axes[0]
        max_points = 50000
        if len(original_nz) > max_points:
            idx = np.random.default_rng(42).choice(
                len(original_nz), max_points, replace=False
            )
        else:
            idx = np.arange(len(original_nz))

        ax.scatter(original_nz[idx], shuffled_nz[idx], alpha=0.2, s=1)
        lims = [
            min(0, shuffled_nz.min()),
            np.percentile(np.concatenate([original_nz, shuffled_nz]), 95),
        ]
        ...
        stats_text = (
            f"Original: mu={original_nz.mean():.6f}, std={original_nz.std():.6f}\n"
            f"Shuffled: mu={shuffled_nz.mean():.6f}, std={shuffled_nz.std():.6f}\n"
            f"Corr: r={global_corr:.4f}"
        )
        ax.text(
            0.05,
            0.95,
        ...
        bins = np.linspace(
            min(0, shuffled_nz.min()),
            np.percentile(np.concatenate([original_nz, shuffled_nz]), 99),
            100,
        )
        ax.hist(
            original_nz,
        ...
        fig.suptitle(
            "Weight Shuffle: Non-zero weights shuffled within cell-type pairs",
            fontsize=14,
            fontweight="bold",
        )
        plt.tight_layout()

        plot_path = output_dir / "weight_shuffle_comparison.png"
        fig.savefig(plot_path, dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"  - Saved weight shuffle comparison plot to {plot_path}")
        ...
        print(f"\nSaved initial shuffled+perturbed state to {initial_state_dir}")
    ...
    if optimisable != "scaling_factors":
        raise NotImplementedError(
            f"shuffled-control supports optimisable='scaling_factors' only; "
            f"got {optimisable!r}"
        )
    ...
    def plot_generator(
        ...
        **kwargs,
        ...
        fig = plot_spike_trains(
            ...
            fraction=1.0,
            random_seed=None,
            title=f"Feedforward Network (shuffled): Target vs Trained (first {n_plot} neurons)",
            ylabel="Neuron",
            figsize=(14, 8),
        )
    ...
    print("Weight manipulation: shuffled within cell-type pairs (no noise)")
