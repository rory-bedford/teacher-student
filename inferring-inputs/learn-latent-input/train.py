"""
Inferring-inputs: learn-latent-input training.

Feedforward inputs are raw continuous OU latent trajectories (not spikes).
The network learns a low-rank latent->neuron mapping (U @ V decomposition)
...
def main(
    ...
    """Train network with learn-latent-input: low-rank latent->neuron mapping + scaling factors.

    Feedforward inputs are continuous OU latent trajectories from spike_data.zarr.
    Phase 3 learns a low-rank latent weight matrix (U @ V) while recovering
    ...
    # absorbed into the learned latent weight constants).
    ...
    # ================================================================
    # Load OU latent dataset to get n_latents
    # ================================================================
    ...
    n_total_inputs = n_latents + n_neurons

    # Feedforward (latent) weights: per-output-cell-type constant initialisation
    # Calibrate so total latent input per neuron ~ teacher's total FF input,
    # separately for excitatory and inhibitory output neurons.
    latent_weights = np.zeros((n_latents, n_neurons), dtype=np.float32)
    latent_mask = np.ones((n_latents, n_neurons), dtype=bool)
    ...
    for type_idx, type_name in enumerate(output_cell_type_names):
        ...
        constant_val = mean_ff_type / n_latents
        latent_weights[:, type_mask] = constant_val
        ff_weight_constants.append(constant_val)
        print(f"  Latent weight init -> {type_name}: {constant_val:.6f}")
    ...
    # is_feedforward_input: first n_latents are feedforward, rest are recurrent
    is_feedforward_input = np.zeros(n_total_inputs, dtype=bool)
    is_feedforward_input[:n_latents] = True

    # Cell type indices for inputs:
    # - Latent inputs get cell type 0 ("latent")
    # - Recurrent inputs get cell types offset by n_ff_cell_types
    latent_cell_type_indices = np.zeros(n_latents, dtype=np.int64)
    ...
    if resume_from is not None:
        ...
        if "recurrent_scaling_factors" in saved_targets:
            ...
            # Backwards compat: old runs saved full (3,2) matrix as feedforward_scaling_factors
    ...
    # ================================================================
        ...
        # Concatenate weights: [latent_weights, recurrent_weights]
        concatenated_weights = np.concatenate([latent_weights, teacher_weights], axis=0)
        concatenated_mask = np.concatenate([latent_mask, recurrent_mask], axis=0)
        concatenated_cell_type_indices = np.concatenate(
            [latent_cell_type_indices, recurrent_cell_type_indices]
        )
        ...
        # absorbed into the learned latent weights. Only perturb recurrent.
        ...
        # Apply perturbation to recurrent weights only (not latent weights)
        perturbed_weights = concatenated_weights.copy()
        for input_idx in range(n_latents, n_total_inputs):
    ...
    print(f"    ({n_latents} latent + {n_neurons} recurrent)")
    ...
    ff_idx_per_ct = {
        name: np.flatnonzero(latent_cell_type_indices == ct_id)
        for ct_id, name in enumerate(ff_input_cell_type_names)
    }

    # Single FF input cell type name (the "latent" channel) — used as the source
    # name for FF projections.
    latent_src_name = ff_input_cell_type_names[0]
    ...
    if wandb_config:
        ...
        run_name = wandb_config.pop("name", None) or (
            output_dir.name if output_dir else "inferring-inputs-learn-latent-input"
        )
    ...
    # Helper: build dict of ScalingFactorProjections covering all pairs
    # (recurrent + latent->rec). FF connectome = uniform 1.0 so that the
    # latent log_sf carries the FF weight constant directly; recurrent
    # connectome = perturbed rec block, with log_sf carrying sf_recurrent[s,t].
    perturbed_rec_block = perturbed_weights[n_latents:, :]
    ...
    def _build_sf_projections(rec_log_sf_2x2, ff_log_constants):
        ...
        for src_id, src_name in enumerate(ff_input_cell_type_names):
            ...
            for tgt_id, tgt_name in enumerate(rec_output_cell_type_names):
                ...
                # Uniform 1.0 connectome over (latent_src, rec_tgt) — full FF.
    ...
    )  # (1, 2) — one latent source, 2 targets
    ...
    if resume_from is not None and cma_state_path.exists():
        ...
        # Build ScalingFactorProjections for all pairs.
        ...
        print("    - 2 FF weight constants (latent->exc, latent->inh)")
        ...
        for tgt_name in rec_output_cell_type_names:
            cma_pair_order.append((latent_src_name, tgt_name))
        ...
        def cma_es_callback(metrics):
            ...
            current_latent_w = current_weights[:n_latents, :]
            for type_idx, type_name in enumerate(output_cell_type_names):
                type_mask = cell_type_indices == type_idx
                log_dict[f"cma_es_ff_weights/{type_name}_constant"] = float(
                    current_latent_w[:, type_mask].mean()
                )
            ...
            latent_chunk_eval = (
                cma_eval_input[0, :, :n_latents].detach().cpu().numpy()
            )  # (time, n_latents)
            rec_spikes_eval = spikes_np  # (time, n_neurons)
            rec_weights_eval = current_weights[n_latents:, :]

            # FF: total latent activation * outgoing weight per latent
            latent_activity_total = latent_chunk_eval.sum(axis=0)
            latent_outgoing_w = current_latent_w.sum(axis=1)
            ff_drive = (latent_activity_total * latent_outgoing_w).sum()
        ...
        for tgt_id, tgt_name in enumerate(rec_output_cell_type_names):
            best_ff_log_constants[0, tgt_id] = float(
                cma_projections[(latent_src_name, tgt_name)].log_sf.detach().cpu()
            )
    ...
    def plot_generator(
        ...
        **kwargs,
        ...
        fig = plot_spike_trains(
            ...
            fraction=1.0,
            random_seed=None,
            title=f"Latent-mapping: Target vs Trained (first {n_plot} neurons)",
            ylabel="Neuron",
            figsize=(14, 8),
        )
    ...
    def stats_computer(snapshot):
        ...
        latent_activations = input_data[:, :n_latents]  # (time, n_latents)
        ...
        # ======================
        # Learned latent weight matrix stats
        # ======================
        ...
        # and LowRankProjection has no per-pair SF — so the latent block is
        # already the effective weight matrix.
        latent_w_effective = weights_ff[:n_latents, :]

        stats["latent_weights/mean"] = float(latent_w_effective.mean())
        stats["latent_weights/std"] = float(latent_w_effective.std())
        stats["latent_weights/min"] = float(latent_w_effective.min())
        stats["latent_weights/max"] = float(latent_w_effective.max())

        # Per output cell type
        for type_idx, type_name in enumerate(recurrent.cell_types.names):
            type_mask = cell_type_indices == type_idx
            if type_mask.sum() > 0:
                w_type = latent_w_effective[:, type_mask]
                stats[f"latent_weights/{type_name}_mean"] = float(w_type.mean())
                stats[f"latent_weights/{type_name}_std"] = float(w_type.std())

        # Effective rank of the learned latent->neuron mapping
        try:
            s = np.linalg.svd(latent_w_effective, compute_uv=False)
            effective_rank = (s.sum() ** 2) / (s**2).sum()
            stats["latent_weights/effective_rank"] = float(effective_rank)
            total_var = (s**2).sum()
            top_var = (s[: min(ff_rank, len(s))] ** 2).sum()
            stats["latent_weights/top_rank_variance_fraction"] = float(
        ...
        # ======================
        rec_weights = weights_ff[n_latents:, :]
        ...
        # FF: total latent activation * outgoing weight per latent
        latent_activity_total = latent_activations.sum(axis=0)  # (n_latents,)
        latent_outgoing_w = latent_w_effective.sum(axis=1)  # (n_latents,)
        ff_drive = (latent_activity_total * latent_outgoing_w).sum()
    ...
    if run_phase2:
        ...
        # Latent pairs' log_sf carries the FF weight constant directly; the
        ...
        for tgt_id, tgt_name in enumerate(rec_output_cell_type_names):
            best_ff_log_constants[0, tgt_id] = float(
                phase2_projections[(latent_src_name, tgt_name)].log_sf.detach().cpu()
            )
        best_ff_weight_constants = np.exp(best_ff_log_constants.ravel()).astype(
            np.float32
        )
    ...
    # ==============================================
    # Initialize Phase 3 Model (Low-Rank Latent Weights)
    # ==============================================
    ...
    #   - LowRankProjection for each FF pair (latent -> rec_ct), initialised
    ...
    # FF pairs: log of the effective latent block per output type is constant
    ...
    for tgt_id, tgt_name in enumerate(rec_output_cell_type_names):
        ...
        # leading one. The latent block is rank-1 (uniform per output type),
        ...
        print(
            f"  FF (latent->{tgt_name}): rank {rank_eff}, "
            f"top-{rank_eff} variance explained {var_explained:.1%}"
        )
        phase3_projections[(latent_src_name, tgt_name)] = LowRankProjection(
            n_source=n_latents,
            n_target=n_tgt,
            rank=rank_eff,
            U_init=U_init,
            V_init=V_init,
            mask=None,  # full latent connectivity
        )
    ...
    print(
        f"  Training {n_neurons} neurons: low-rank latent weights (rank {ff_rank}) + recurrent scaling factors"
    )
    ...
    np.savez(
        ...
        recurrent_weights=final_weights[n_latents:, :],
        feedforward_connectivity=concatenated_mask,
