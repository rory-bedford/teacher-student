"""
Inferring-inputs: ou-rates training with CMA-ES initialisation.

Identical to fully-observed training except that feedforward inputs are
Poisson-sampled from the OU process rates saved with the teacher activity,
rather than loaded as pre-recorded spike counts.  The recurrent (output)
spike trains are still loaded exactly from disk as training targets.
...
def main(
    ...
    """Train all neurons to match target spike train activity using OU-rate-driven inputs.

    Feedforward inputs are Poisson-sampled from the OU process rates saved in
    spike_data.zarr rather than loaded as exact pre-recorded spikes.
    Two-phase training:
    ...
    spike_dataset = OUReconstructedFFDataset(
    ...
    # Split perturbed_weights into FF block and recurrent block. The chunked-FF
    # projection builders expect them separately.
    ...
    if wandb_config:
        ...
        run_name = wandb_config.pop("name", None) or (
            output_dir.name if output_dir else "inferring-inputs-ou-rates"
        )
    ...
    if resume_from is not None and cma_state_path.exists():
        ...
        # perturbation-baked) per-pair block; init SF = teacher's configured
    ...
    # Gradient phase: per-pair ScalingFactorProjection. ``best_scaling_factors_FF``
    # (shape (n_ff_ct + n_rec_ct, n_rec_ct)) provides the per-pair initial SF.
    if optimisable != "scaling_factors":
        raise NotImplementedError(
            f"ou-rates supports optimisable='scaling_factors' only; got {optimisable!r}"
        )
    ...
    def plot_generator(
        ...
        **kwargs,
        ...
        fig = plot_spike_trains(
            ...
            fraction=1.0,
            random_seed=None,
            title=f"OU-rates: Target vs Trained (first {n_plot} neurons)",
            ylabel="Neuron",
            figsize=(14, 8),
        )
